...
func TestContainsInCollection(t *testing.T) {
	arr := Collect([]int{3, 6, 8, 9})
	if !arr.Contains(8) {
		t.Errorf("TestLastCollection: expected: true, got: %v", false)
	}
	if arr.Contains(10) {
		t.Errorf("TestLastCollection: expected: false, got: %v", true)
	}
}
